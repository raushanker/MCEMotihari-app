...
export default function EditOlxScreen() {
  const theme = useThemeColors();
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const user = useAppStore(state => state.user);
  const { editItem, items } = useOlxStore();
  ...
  useEffect(() => {
    if (id) {
      ...
      if (existingItem) {
        ...
        if (existingItem.imageUrl) setImageUrl(existingItem.imageUrl);
      }
    }
  }, [id, items]);
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (imageUrl) {
        ...
        itemData.imageUrl = ""; // Clear image if removed
      }

      await editItem(id as string, itemData);
    ...
    >
      <View style={[styles.headerContainer, { paddingTop: insets.top + 10, backgroundColor: theme.backgroundElement, borderBottomColor: theme.cardBorder }]}>
        <View style={styles.headerContent}>
          ...
          <Text style={[styles.headerTitle, { color: theme.text, flex: 1, marginLeft: 12 }]}>Edit Item</Text>
          <TouchableOpacity 
          ...
          >
            {isSubmitting ? (
              ...
              <Text style={[styles.postBtnText, { color: theme.isDark ? '#3B82F6' : theme.primary }]}>Update</Text>
            )}
          </TouchableOpacity>
        </View>
      </View>
