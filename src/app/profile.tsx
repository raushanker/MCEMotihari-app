...
const ExploreProfileScreen = React.memo(function ExploreProfileScreen() {
  ...
  const [isProfileTraditionalLoggingIn, setIsProfileTraditionalLoggingIn] =
  ...
  const [profileLoginStage, setProfileLoginStage] = React.useState<
    "welcome" | "credentials"
  >("welcome");
    ...
    try {
      ...
      if (result.success) {
        if (result.isNewUser) {
          ...
          showPremiumAlert(
            "Welcome Back! 🎉",
            "Apka session successfully restore ho chuka hai.",
            "success"
          );
        }
      }
  ...
  const handleProfileTraditionalLoginSubmit = async () => {
    ...
    if (!cleanId) {
      showPremiumAlert(
        "Required Field",
        "Kripya apna email ya mobile number darj karein.",
        "warning"
      );
      return;
    }
    if (!cleanPass) {
      showPremiumAlert(
        "Required Field",
        "Kripya apna password darj karein.",
        "warning"
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        showPremiumAlert(
          "Welcome Back! 🎉",
          "MCE Connect dashboard me aapka swagat hai.",
          "success"
        );
      } else {
        showPremiumAlert(
      ...
      showPremiumAlert(
        "Sign In Failed",
        err?.message || "Login request error.",
        "error"
      );
    } finally {
  ...
  const usernameLockRemainingText = React.useMemo(() => {
    ...
    const remainingDays = Math.ceil(
      (fourteenDaysInMs - timeDiff) / (24 * 60 * 60 * 1000)
    );
  ...
  const [expandedLinkSection, setExpandedLinkSection] = useState<
    "social" | "custom"
  >("social");
  ...
  // Headline state for Edit Profile
  ...
  const [editCustomLinks, setEditCustomLinks] = useState<
  ...
  React.useEffect(() => {
    ...
    if (!/^[a-z0-9_]{3,20}$/.test(cleanUser)) {
      ...
      setUsernameMessage(
        "Username me sirf chote letters, numbers aur underscores ho sakte hain (3-20 characters)!"
      );
      return;
    }
    if (!/[a-z]/.test(cleanUser)) {
      setUsernameStatus("invalid");
      setUsernameMessage(
        "Username me kam se kam ek letter (a-z) hona zaroori hai!"
      );
      return;
    }
    ...
    if (digitCount < 2) {
      setUsernameStatus("invalid");
      setUsernameMessage(
        "Username me kam se kam 2 numbers (digits) hona zaroori hai!"
      );
      return;
    }
      ...
      try {
        ...
        if (usernameDocSnap.exists()) {
          ...
          if (claimedUid !== user?.uid) {
            ...
            setUsernameMessage(
              "❌ Ye username pehle se kisi aur ne le rakha hai!"
            );
          } else {
            setUsernameStatus("available");
        ...
        setUsernameStatus("idle");
        setUsernameMessage(
          "Error checking availability. Kripya connection check karein."
        );
      }
    }, 450);
  ...
  const openEditProfile = () => {
    if (user) {
      ...
      setEditHeadline(user.vibeStatus || "");
  ...
  const shouldOpenLoginSettings = useAppStore(
    (state) => state.shouldOpenLoginSettings
  );
  const setShouldOpenLoginSettings = useAppStore(
    (state) => state.setShouldOpenLoginSettings
  );
  ...
  const shouldOpenEditProfile = useAppStore(
    (state) => state.shouldOpenEditProfile
  );
  const setShouldOpenEditProfile = useAppStore(
    (state) => state.setShouldOpenEditProfile
  );
    ...
    if (result.uri) {
      ...
      try {
        ...
        if (cloudinaryUrl) {
          ...
          showPremiumAlert(
            "Success 🎉",
            "Image Cloudinary par successfully upload ho chuki hai!",
            "success"
          );
        }
      } catch (err) {
  ...
  const handleSaveAvatar = async () => {
    ...
    try {
      ...
      await setDoc(
        userDocRef,
        sanitizeFirestoreData({ photoUrl: finalPhoto }),
        { merge: true }
      );
    ...
    if (Platform.OS === "web") {
      ...
      if (confirm) {
        try {
          ...
          await setDoc(
            userDocRef,
            sanitizeFirestoreData({ isPrivate: newIsPrivate }),
            { merge: true }
          );
          const { setUser } = useAppStore.getState();
  ...
  const validateBatch = (batch: string) => {
    ...
    if (match[2]) {
      ...
      const endYear =
        endYearStr.length === 2
          ? parseInt(match[1].substring(0, 2) + endYearStr, 10)
  ...
  const validateRollNo = (rollNo: string) => {
    ...
    if (!/^\d{5}$/.test(trimmed))
      return "Please enter a valid 5-digit MCE roll number.";
  ...
  const validateRegNo = (regNo: string) => {
    ...
    if (!/^\d{11}$/.test(trimmed))
      return "Please enter a valid 11-digit registration number.";
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (rollErr) {
        showPremiumAlert("Invalid Roll Number", rollErr, "warning");
        return;
      }

      const regErr = validateRegNo(editRegNo);
      if (regErr) {
        showPremiumAlert("Invalid Registration", regErr, "warning");
        return;
      }
      ...
      if (editRole === "Student") {
        if (
          !editRollNo.trim() ||
          editRollNo.trim().length !== 5 ||
          isNaN(Number(editRollNo.trim()))
        ) {
          showPremiumAlert(
            "Required Field",
            "Student ke liye MCE Roll Number (strictly 5 digits) required hai!",
            "warning"
          );
          return;
        }
        if (
          !editRegNo.trim() ||
          editRegNo.trim().length !== 11 ||
          isNaN(Number(editRegNo.trim()))
        ) {
          showPremiumAlert(
            "Required Field",
            "Student ke liye Registration Number (strictly 11 digits) required hai!",
            "warning"
          );
          return;
        }
        if (!editBatch.trim()) {
          showPremiumAlert(
            "Required Field",
            "Student ke liye Academic Batch Years required hai!",
            "warning"
          );
          return;
        }
        ...
        if (batchErr) {
          showPremiumAlert("Invalid Batch", batchErr, "warning");
          return;
        }
      ...
      if (result.success) {
        ...
        showPremiumAlert(
          "Failed to Save",
          result.error || "Failed to save academic profile.",
          "error"
        );
      }
    } catch (e: any) {
      ...
      showPremiumAlert(
        "Failed to Save",
        e?.message || "Academic profile save karne me error aaya.",
        "error"
      );
    } finally {
      setIsSaving(false);
  ...
  const handleSavePassword = async () => {
    ...
    if (phoneChanged || passwordChanged) {
      ...
      if (!cleanPhone) {
        showPremiumAlert(
          "Missing Fields",
          "Kripya apna mobile number darj karein.",
          "warning"
        );
        return;
      }
      if (cleanPhone.length !== 10 || isNaN(Number(cleanPhone))) {
        showPremiumAlert(
          ...
          "Kripya ek sahi 10-digit mobile number type karein.",
          "warning"
        );
        return;
      }
      ...
      if (needsPassword && !cleanPass) {
        showPremiumAlert(
          "Missing Password",
          "Kripya account secure karne ke liye ek password banayein.",
          "warning"
        );
        return;
      }
    ...
    const proceedToSave = async () => {
      ...
      try {
        ...
        if (cleanUser && cleanUser !== user?.username) {
          if (usernameStatus === "taken" || usernameStatus === "invalid") {
            showPremiumAlert(
              ...
              usernameMessage || "Ye username available nahi hai.",
              "warning"
            );
            return;
          }
          if (usernameStatus === "checking") {
            showPremiumAlert(
              ...
              "Username check kiya ja raha hai, kripya thoda ruken.",
              "info"
            );
            return;
          }
          ...
          if (!result.success) {
            showPremiumAlert(
              ...
              result.error || "Failed to save name.",
              "error"
            );
            return;
          }
        ...
        showPremiumAlert(
          "Error",
          error?.message || "Credentials save karne me error aaya.",
          "error"
        );
      } finally {
        setIsSaving(false);
    ...
    if (cleanUser && cleanUser !== user?.username) {
      Alert.alert(
        ...
        `Aap apna username badalkar "${cleanUser}" kar rahe hain.\n\n⚠️ IMPORTANT: Ek baar badalne ke baad, aap agle 14 din tak ise dobara change nahi kar payenge!\n\nKya aap sure hain?`,
        [
          { text: "Cancel", style: "cancel" },
  ...
  const handleSaveSkills = async () => {
    ...
    try {
      ...
      await setDoc(
        ...
        sanitizeFirestoreData({ skills: editSkills }),
        { merge: true }
      );
      ...
      useAppStore
        .getState()
        .showToast("Tech skills updated successfully! 🎉", "success");
    } catch (e: any) {
  ...
  const handleSaveLinks = async () => {
    ...
    for (const link of linksToValidate) {
      if (
        link.value &&
        !urlPattern.test(link.value.replace(/^https?:\/\//i, ""))
      ) {
        showPremiumAlert(
          "Invalid URL Format 🌐",
    ...
    try {
      ...
      useAppStore
        .getState()
        .showToast(
          "Social profiles & links updated successfully! 🎉",
          "success"
        );
    } catch (e: any) {
    ...
    if (Platform.OS === "web") {
      ...
      if (confirm) {
        (async () => {
          if (!user) return;
          setIsSaving(true);
          const updatedExps = (user.experiences || []).filter(
            (e: any) => e.id !== expId
          );
          try {
            const { doc, setDoc } = require("firebase/firestore");
            ...
            await setDoc(
              doc(db, "publicProfiles", user.uid),
              sanitizeFirestoreData({ experiences: updatedExps }),
              { merge: true }
            );
  ...
  const validateExperience = (
    ...
    if (!isCurr) {
      ...
      if (
        ...
        return (
          "End Year must be between " + startYearNum + " and " + currentYear
        );
      }
    }
    return null;
  ...
  const handleSaveExperience = async () => {
    ...
    const expValError = validateExperience(
      expRole.trim(),
      expCompany.trim(),
      expDesc.trim(),
      expStartYear.trim(),
    ...
    if (editingExpId) {
      updatedExps = (user.experiences || []).map((exp: Experience) =>
        exp.id === editingExpId ? { ...newExp, id: editingExpId } : exp
      );
    } else {
  ...
  const handleSaveEducation = async () => {
    ...
    if (!eduIsCurrent && !eduEndYear.trim()) {
      showPremiumAlert(
        "Missing Fields",
        "Please select an end year or mark as current.",
        "warning"
      );
      return;
    }

    const valError = validateEducation(
      eduSchool,
      eduDegree,
      eduFieldOfStudy,
      eduStartYear,
    ...
    let updatedEdu: any[];
    if (editingEduId) {
      updatedEdu = (user.education || []).map((edu: any) =>
        edu.id === editingEduId ? { ...newEdu, id: editingEduId } : edu
      );
    } else {
    ...
    try {
      ...
      await setDoc(
        doc(db, "publicProfiles", user.uid),
        sanitizeFirestoreData({ education: updatedEdu }),
        { merge: true }
      );
  ...
  const handleSavePublication = async () => {
    ...
    const valError = validatePublication(
      pubTitle,
      pubPublisher,
      pubDate,
      pubUrl,
    ...
    let updatedPubs: any[];
    if (editingPubId) {
      updatedPubs = (user.publications || []).map((pub: any) =>
        pub.id === editingPubId ? { ...newPub, id: editingPubId } : pub
      );
    } else {
    ...
    try {
      ...
      await setDoc(
        doc(db, "publicProfiles", user.uid),
        sanitizeFirestoreData({ publications: updatedPubs }),
        { merge: true }
      );
      ...
      useAppStore
        ...
        .showToast(
          editingPubId
            ? "Conference/Publication updated successfully! 🎉"
    ...
    if (Platform.OS === "web") {
      ...
      if (confirm) {
        (async () => {
          if (!user) return;
          setIsSaving(true);
          const updatedEdu = (user.education || []).filter(
            (e: any) => e.id !== eduId
          );
          try {
            const { doc, setDoc } = require("firebase/firestore");
    ...
    if (Platform.OS === "web") {
      ...
      if (confirm) {
        (async () => {
          if (!user) return;
          setIsSaving(true);
          const updatedPubs = (user.publications || []).filter(
            (e: any) => e.id !== pubId
          );
          try {
            const { doc, setDoc } = require("firebase/firestore");
    ...
    if (Platform.OS === "web") {
      ...
      if (confirm) {
        try {
          ...
          alert(
            "Default email app open karne me error aaya. Kripya mcemotihari.tech@gmail.com par direct mail karein!"
          );
        }
      }
      return;
  ...
  const handleOpenLink = async (url?: string) => {
    ...
    try {
      ...
      if (supported) {
        ...
        showPremiumAlert(
          "Unable to open URL",
          `The URL pattern "${formattedUrl}" is not supported.`,
          "warning"
        );
      }
    } catch (error) {
        ...
        >
          <ScrollView
            contentContainerStyle={[
              ...
              { paddingBottom: Platform.OS === "android" ? 240 : 130 },
            ]}
          ...
          >
            ...
            <View style={styles.loginHeaderContainer}>
              ...
              />
              <Text style={[styles.loginCollegeName, { color: theme.text }]}>
                MCE Motihari
              </Text>
            ...
            >
              {profileLoginStage === "welcome" ? (
                ...
                >
                  ...
                  >
                    Motihari College of Engineering digital campus. Connect with
                    batchmates, view verified notices, and search student
                    profiles.
                  </Text>
                  ...
                  >
                    {isProfileLoggingIn ? (
                      ...
                      <>
                        ...
                        <Text style={styles.loginGoogleBrandBtnText}>
                ...
                <View style={{ width: "100%" }}>
                  ...
                  <View style={styles.loginInputContainer}>
                    <Text
                      style={[styles.loginInputLabel, { color: theme.text }]}
                    >
                      Email, Username or Phone
                    </Text>
                    <View
                      style={[
                        styles.loginInputFieldContainer,
                        {
                          backgroundColor: theme.background,
                          borderColor: theme.cardBorder,
                        },
                  ...
                  <View style={styles.loginInputContainer}>
                    ...
                    >
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          setIsProfilePasswordVisible(!isProfilePasswordVisible)
                        }
            ...
            <Text style={styles.loginInfoText}>
              *Sign in allows students & alumni to write posts, send connection
              requests, and access materials.
            </Text>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      />
      <View
        style={{ height: insets.top, backgroundColor: theme.backgroundElement, zIndex: 100 }}
      />
      ...
      >
        ...
        >
          <TouchableOpacity
            ...
            onPress={user.role !== "Guest" ? openPhotoModal : undefined}
            activeOpacity={0.85}
          >
          ...
          {user.role !== "Guest" && (
            ...
            >
              <Ionicons
                name={user.isPrivate ? "lock-closed" : "globe"}
                size={14}
                color={user.isPrivate ? "#EF4444" : "#22C55E"}
                style={{ marginRight: 4 }}
              />
              <Text
          ...
          {(() => {
            ...
            if (links.website && links.website.trim())
              activeLinksList.push({ key: "website", value: links.website });
            if (links.linkedin && links.linkedin.trim())
              activeLinksList.push({ key: "linkedin", value: links.linkedin });
            if (links.github && links.github.trim())
              activeLinksList.push({ key: "github", value: links.github });
            ...
            if (links.youtube && links.youtube.trim())
              activeLinksList.push({ key: "youtube", value: links.youtube });
            if (links.facebook && links.facebook.trim())
              activeLinksList.push({ key: "facebook", value: links.facebook });
            if (links.twitter && links.twitter.trim())
              activeLinksList.push({ key: "twitter", value: links.twitter });
              ...
              >
                {activeLinksList.map((item) => {
                  ...
                  if (
                    ...
                    item.key === "googlescholar" ||
                    item.key.includes("scholar")
                  ) {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleOpenExternalLinkWithConfirmation(item.value)
                      }
                      activeOpacity={0.7}
                    >
                  ...
                  <>
                    <View style={styles.credentialRow}>
                      <View style={styles.credentialHalf}>
                        ...
                        <Text
                          style={[styles.credentialVal, { color: theme.text }]}
                        >
                          {user.batch || "N/A"}
                        </Text>
                      </View>
                        ...
                        <View style={styles.credentialHalf}>
                          ...
                          >
                            ...
                            >
                              ...
                              <TouchableOpacity
                                onPress={() => setIsRollVisible(!isRollVisible)}
                                style={{ padding: 4 }}
                                activeOpacity={0.7}
                              >
                                <Ionicons
                      ...
                      <View style={styles.credentialItem}>
                        ...
                        >
                          ...
                          >
                            ...
                            <TouchableOpacity
                              onPress={() => setIsRegVisible(!isRegVisible)}
                              style={{ padding: 4 }}
                              activeOpacity={0.7}
                            >
                              <Ionicons
          ...
          >
            <View style={styles.cardHeader}>
              ...
              <TouchableOpacity
                style={styles.cardEditBtn}
                onPress={() =>
                  useAppStore.getState().setCreatePostVisible(true)
                }
              >
            ...
            {myPosts.length === 0 ? (
              <View style={styles.emptyTimelineContainer}>
                ...
                >
                  No campus thoughts shared yet. Tap the '+' icon above to write
                  your first post!
                </Text>
              </View>
            ) : (
                  ...
                  {myPosts.slice(0, 10).map((post) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        activeOpacity={0.85}
                        onPress={() =>
                          router.push(`/post/${post.id}?from=profile`)
                        }
                        style={{
                          width: width - 80,
                          backgroundColor: theme.background,
              ...
              >
                {!user.skills?.length && (
                  ...
                  >
                    ...
                    <Text
                      style={[styles.quickAddPillText, { color: "#9333EA" }]}
                    >
                      Skills
                    </Text>
                  </TouchableOpacity>
                ...
                {!user.experiences?.length && (
                  ...
                  >
                    ...
                    <Text
                      style={[styles.quickAddPillText, { color: "#2563EB" }]}
                    >
                      Experience
                    </Text>
                  </TouchableOpacity>
                ...
                {!user.links?.github &&
                  ...
                  !user.links?.instagram && (
                    ...
                    >
                      ...
                      <Text
                        style={[styles.quickAddPillText, { color: "#0891B2" }]}
                      >
                        @social
                      </Text>
                    </TouchableOpacity>
            ...
            >
              ...
              <View style={styles.tagGrid}>
                {user.skills.map((skill: string, index: number) => (
                  ...
                  >
                    <Text style={[styles.skillTagText, { color: "#9333EA" }]}>
                      {skill}
                    </Text>
                  </View>
                ))}
          ...
          {user.experiences && user.experiences.length > 0 && (
            ...
            >
                ...
                {user.experiences.slice(0, 3).map((exp: Experience) => (
                  ...
                  >
                    ...
                    <View style={styles.experienceDetails}>
                      <Text
                        style={[styles.experienceRole, { color: theme.text }]}
                      >
                        {exp.role.replace(/,\\s*$/, "")}
                      </Text>
                      <Text
                      ...
                      >
                        {exp.company} •{" "}
                        <Text style={styles.experienceTypeTag}>
                          {exp.employmentType}
                        </Text>
                      </Text>
              ...
              {user.experiences.length > 3 && (
                <TouchableOpacity
                  style={[
                    styles.viewAllPostsBtn,
                    { borderColor: theme.cardBorder, marginTop: 12 },
                  ]}
                  onPress={() => setIsAllExperiencesVisible(true)}
                  activeOpacity={0.8}
                >
                  <Text
                    style={[styles.viewAllPostsBtnText, { color: theme.text }]}
                  >
              ...
              >
                ...
                >
                  ...
                  <View style={styles.tabContainer}>
                    {(["Student", "Alumni", "Faculty", "Other"] as const).map(
                      (r) => (
                        <TouchableOpacity
                          key={r}
                          style={[
                            styles.tab,
                            editRole === r && styles.activeTab,
                          ]}
                        ...
                        >
                          <Text
                            style={[
                              styles.tabText,
                              editRole === r && styles.activeTabText,
                              { fontSize: 10.5 },
                            ]}
                          >
                            {r === "Other" ? "Other" : r}
                  ...
                  {(editRole === "Student" || editRole === "Alumni") && (
                    <>
                      ...
                      <View style={styles.inputGroup}>
                        <Text style={styles.modalLabel}>
                          Academic Batch Years * (e.g. 2020-2024)
                        </Text>
                        <TextInput
                          style={styles.modalInput}
                  ...
                  {(editRole === "Student" ||
                    ...
                    <View style={styles.inputGroup}>
                      ...
                      <ScrollView
                        horizontal
                        showsHorizontalScrollIndicator={false}
                        style={styles.modalDeptScroller}
                      >
                        {DEPARTMENTS.map((dept) => (
                          ...
                          >
                            <Text
                              style={[
                                styles.modalDeptText,
                                editDept === dept && styles.activeModalDeptText,
                              ]}
                            >
                              {dept}
                            </Text>
                    ...
                    >
                        ...
                        >
                          <View>
                            ...
                            <Text style={{ fontSize: 10.5, color: "#94A3B8" }}>
                              Hide graduation year from public profile
                            </Text>
                          </View>
                          <TouchableOpacity
                            onPress={() =>
                              setEditIsBatchPrivate(!editIsBatchPrivate)
                            }
                          >
                            <Ionicons
                              name={
                                editIsBatchPrivate ? "toggle" : "toggle-outline"
                              }
                              size={32}
                              color={editIsBatchPrivate ? "#F97316" : "#CBD5E1"}
                            />
                          </TouchableOpacity>
                        </View>
                      )}
                      ...
                      >
                        <View>
                          ...
                          <Text style={{ fontSize: 10.5, color: "#94A3B8" }}>
                            Hide engineering branch from public profile
                          </Text>
                        </View>
                        <TouchableOpacity
                          onPress={() =>
                            setEditIsDeptPrivate(!editIsDeptPrivate)
                          }
                        >
                          <Ionicons
                            name={
                              editIsDeptPrivate ? "toggle" : "toggle-outline"
                            }
                            size={32}
                            color={editIsDeptPrivate ? "#F97316" : "#CBD5E1"}
                          />
                        </TouchableOpacity>
                      </View>
                    </View>
                  ...
                  {editRole === "Other" && (
                    ...
                    >
                      ...
                      >
                        🛡️ Other Role: No academic batch, roll number or
                        department registration is required.
                      </Text>
                    </View>
                  )}
              ...
              >
                ...
                >
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    >
                      Email ID change nahi ho sakti, ye aapka primary sign-in ID
                      hai.
                    </Text>
                  </View>
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    {user.phone ? (
                      ...
                      >
                        Current: +91 {user.phone.slice(0, 3)}******
                        {user.phone.slice(9)}
                      </Text>
                    ) : (
                      <Text
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    >
                      <TextInput
                        ...
                        placeholder={
                          user.hasPassword
                            ? "Enter new password to change or leave empty"
                        ...
                        onFocus={() => {
                          setSafeTimeout(() => {
                            configScrollViewRef.current?.scrollToEnd({
                              animated: true,
                            });
                          }, 150);
                        }}
              ...
              >
                ...
                >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setIsCustomPhotoUrlVisible((visible) => !visible)
                    }
                    activeOpacity={0.8}
                  >
                    ...
                    <Ionicons
                      name={
                        isCustomPhotoUrlVisible ? "chevron-up" : "chevron-down"
                      }
                      size={16}
                      color={theme.textSecondary}
                    />
      ...
      {isHolidaysVisible && (
        <HolidaysModal
          visible={isHolidaysVisible}
          onClose={() => setIsHolidaysVisible(false)}
        />
      )}
      {isPrivacyVisible && (
        <PrivacyModal
      ...
      {isAllPostsModalVisible && (
        <Modal
          visible={isAllPostsModalVisible}
          animationType="slide"
          transparent={false}
          onRequestClose={() => setIsAllPostsModalVisible(false)}
        >
          <View style={{ flex: 1, backgroundColor: theme.background }}>
            ...
            >
              <TouchableOpacity
                onPress={() => setIsAllPostsModalVisible(false)}
                style={{ padding: 4 }}
                activeOpacity={0.7}
              >
                ...
                >
                  ...
                  >
                    ...
                    {post.imageUrl ? (
                      <Image
                        source={{
                          uri: getOptimizedImageUrl(post.imageUrl, 200),
                        }}
                        style={{
                          width: 60,
                          height: 60,
                  ...
                  <Text style={{ fontSize: 11, color: theme.textSecondary }}>
                    {getFormattedPostTime(post.createdAt, post.timestamp)} • ❤️{" "}
                    {post.claps} Hearts • {post.commentsCount} Comments
                  </Text>
                </TouchableOpacity>
              )}
            />
          ...
          >
            ...
            >
              <View style={styles.actionSheetHeader}>
                ...
                <Text
                  style={[
                    styles.actionSheetSub,
                    { color: theme.textSecondary },
                  ]}
                >
                  Choose an extra academic section to build your profile card
                </Text>
              </View>
              ...
              <View style={styles.actionSheetOptions}>
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.actionSheetBtnText, { color: theme.text }]}
                  >
                    + Education
                  </Text>
                </TouchableOpacity>
      ...
      {isAllExperiencesVisible && (
        <Modal
          visible={isAllExperiencesVisible}
          animationType="slide"
          transparent
          onRequestClose={() => setIsAllExperiencesVisible(false)}
        >
          <View
            style={[
              styles.modalBg,
          ...
          >
            ...
            >
              ...
              >
                <Text style={[styles.modalTitle, { color: theme.text }]}>
                  All Experiences ({user?.experiences?.length || 0})
                </Text>
                <TouchableOpacity
                  onPress={() => setIsAllExperiencesVisible(false)}
                  activeOpacity={0.8}
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            if (isSelected) {
                              setEditSkills(
                                editSkills.filter((s) => s !== presetSkill)
                              );
                            } else {
                        ...
                        >
                          <Text
                            style={[
                              ...
                              { color: theme.textSecondary },
                              isSelected && styles.presetTagTextSelected,
                            ]}
                          >
                            {presetSkill}
                          </Text>
            ...
            <View style={{ flex: 1, backgroundColor: theme.background }}>
              ...
              >
                ...
                >
                  ...
                  >
                    Connect your professional channels to build credibility,
                    showcase research citations, and share B.Tech achievements
                    across the MCE Connect network.
                  </Text>
                </View>
                ...
                >
                  ...
                  {editCustomLinks.length > 0 ? (
                    ...
                    >
                      No custom links added yet. Build your profile by adding
                      unique links!
                    </Text>
                  )}
                  ...
                  >
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        if (
                          !newCustomLinkTitle.trim() ||
                          !newCustomLinkUrl.trim()
                        ) {
                          showPremiumAlert(
                            "Required Fields",
                            "Kripya dono Title aur URL fill karein!",
                            "warning"
                          );
                          return;
                        }
              ...
              >
                ...
                >
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    <View style={styles.tabContainer}>
                      {(["Full-time", "Part-time", "Internship"] as const).map(
                        (type) => (
                          <TouchableOpacity
                            key={type}
                            style={[
                              styles.tab,
                              expEmpType === type && styles.activeTab,
                            ]}
                          ...
                          >
                            <Text
                              style={[
                                styles.tabText,
                                expEmpType === type && styles.activeTabText,
                              ]}
                            >
                              {type}
                            </Text>
                  ...
                  <View style={{ flexDirection: "row", gap: 12 }}>
                    ...
                    <View style={{ width: 120 }}>
                      ...
                      <TextInput
                        ...
                        value={expStartYear}
                        onChangeText={(text) =>
                          setExpStartYear(text.replace(/[^0-9]/g, ""))
                        }
                        keyboardType="numeric"
                        maxLength={4}
                      />
                    ...
                    >
                      ...
                      <View style={{ width: 120 }}>
                        ...
                        <TextInput
                          ...
                          value={expEndYear}
                          onChangeText={(text) =>
                            setExpEndYear(text.replace(/[^0-9]/g, ""))
                          }
                          keyboardType="numeric"
                          maxLength={4}
                        />
                ...
                >
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    <TextInput
                      ...
                      value={eduStartYear}
                      onChangeText={(text) =>
                        setEduStartYear(text.replace(/[^0-9]/g, ""))
                      }
                      keyboardType="numeric"
                      maxLength={4}
                    />
                  ...
                  {!eduIsCurrent && (
                    <View style={styles.inputGroup}>
                      ...
                      <TextInput
                        ...
                        value={eduEndYear}
                        onChangeText={(text) =>
                          setEduEndYear(text.replace(/[^0-9]/g, ""))
                        }
                        keyboardType="numeric"
                        maxLength={4}
                      />
                  ...
                  <View style={styles.inputGroup}>
                    <Text style={styles.modalLabel}>
                      Description / Specializations (Optional)
                    </Text>
                    <TextInput
                      style={[
                        styles.modalInput,
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <View style={styles.inputGroup}>
                    <Text style={styles.modalLabel}>
                      Co-Authors / Contributors (Optional)
                    </Text>
                    <TextInput
                      style={[
                        styles.modalInput,
...
const styles = StyleSheet.create({
  ...
  profileHeaderCard: {
    ...
    alignItems: "flex-start",
    boxShadow:
      Platform.OS === "web" ? `${0}px ${6}px ${10}px #0F172A` : undefined,
  ...
  editBtn: {
    ...
    alignItems: "center",
    boxShadow:
      Platform.OS === "web" ? `${0}px ${2}px ${4}px #F97316` : undefined,
  ...
  hubBlock: {
    ...
    marginHorizontal: 16,
    boxShadow:
      Platform.OS === "web" ? `${0}px ${4}px ${8}px #0F172A` : undefined,
  ...
  saveSubmitBtn: {
    ...
    alignItems: "center",
    boxShadow:
      Platform.OS === "web" ? `${0}px ${4}px ${8}px #F97316` : undefined,
  ...
  signInGoogleBtn: {
    ...
    marginTop: 18,
    boxShadow:
      Platform.OS === "web" ? `${0}px ${4}px ${10}px #F97316` : undefined,
  ...
  headerEditCircleBtn: {
    ...
    alignItems: "center",
    boxShadow:
      Platform.OS === "web" ? `${0}px ${2}px ${3}px #F97316` : undefined,
  ...
  guestCTASignInBtn: {
    ...
    paddingHorizontal: 24,
    boxShadow:
      Platform.OS === "web" ? `${0}px ${3}px ${5}px #F97316` : undefined,
