...
export default function VerifiedAdminsScreen() {
  ...
  const fetchVerifiedAdmins = async () => {
    ...
    try {
      ...
      if (!combined[masterAdminUid]) {
        try {
          ...
          combined[masterAdminUid] = {
            ...
            isVerified: pub.exists() ? pub.data().isVerified : true, // Ensure master admin is considered verified
          };
        } catch {}
      }
      ...
      // 3. Filter ONLY Verified or Super Admins
      const verifiedList = Object.values(combined).filter(
        admin => admin.isVerified || admin.adminType === 'Super Admin' || admin.id === masterAdminUid
      );

      setAdmins(verifiedList);
    } catch (e) {
      console.warn('fetchVerifiedAdmins error', e);
    } finally {
      setLoading(false);
    }
  };
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ marginLeft: 12, flexShrink: 1 }}>
          <Text style={styles.title} numberOfLines={2}>Verified Admins</Text>
          <Text style={styles.subtitle} numberOfLines={2}>Admins with blue tick or Super status</Text>
        </View>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <Text style={styles.emptyText}>No verified admins found.</Text>
            </View>
          }
          renderItem={({ item }) => (
